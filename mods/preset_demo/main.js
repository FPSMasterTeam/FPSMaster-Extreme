...
//
//   F7  fullbright          F8  block outline       F9  chunk borders
//   [   entity boxes        ]   cycle nametag scale  \  cycle particle density

/// <reference path="../recraft.d.ts" />

const s = { fullbright: false, outline: false, chunks: false, ebox: false };
const nametags = [1.0, 2.0, 0.5];
const densities = [1.0, 0.2, 3.0];
let ni = 0;
let di = 0;

recraft.onLoad(() => recraft.log("preset_demo: F7 fullbright, F8 outline, F9 chunks, [ ebox, ] nametag, \\ density"));
...
recraft.onKey((e) => {
  ...
  switch (e.key) {
    ...
    case "F8":
      s.outline = !s.outline;
      recraft.blockOutline(s.outline);
      return true;
    case "F9":
      s.chunks = !s.chunks;
    ...
    case "BracketLeft":
      s.ebox = !s.ebox;
      recraft.entityBox("", [1.0, 0.3, 0.3], s.ebox);
      return true;
    case "BracketRight":
