...
```text
crates/
  fpsmaster_core      internal game state: world, chunks, entities, player physics
  fpsmaster_protocol  packet framing and version-specific protocol code
  fpsmaster_render    wgpu renderer, chunk mesh generation, block atlas
  fpsmaster_app       desktop app, input loop, network thread, game tick
```
...
```text
Server TCP packets
  -> fpsmaster_protocol::v1_8_9 packet decoder
  -> fpsmaster_app network events
  -> fpsmaster_core World / EntityState
  -> fpsmaster_render chunk mesh
  -> GPU draw calls
```
...
`ProtocolVersion` currently contains only `V1_8_9` / protocol 47. New versions should add version-specific modules under `fpsmaster_protocol/src/vX_Y_Z` and translate packet payloads into stable app/core events.
