...
The stable plugin ABI for **recraft** native (`cdylib`) extensions.

A native recraft mod compiles against this crate (plus `abi_stable`) and exports a
root module the host loads with an `abi_stable` layout + version check. This crate
depends only on `abi_stable` — never on recraft internals — so the host can
...
function pointers) using a small JSON protocol — the same vocabulary as the recraft
JS extension layer.

The full SDK (JS + native, the JSON schema, typings, worked examples, a starter
template) lives in the recraft repo's `sdk/` folder.
