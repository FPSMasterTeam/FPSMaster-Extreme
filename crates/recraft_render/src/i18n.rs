...
//! back per-key, exactly like vanilla. Recraft-specific keys (menus, screens
...
/// Recraft-specific strings (keys absent from the vanilla tables), one embedded
/// overlay per language we maintain. Layered over the vanilla `.lang`.
const OVERLAY_EN: &str = include_str!("lang/recraft.en_us.lang");
const OVERLAY_ZH: &str = include_str!("lang/recraft.zh_cn.lang");
...
struct Locale {
    ...
    /// hardcoded English block/item/enchantment/attribute names that recraft
