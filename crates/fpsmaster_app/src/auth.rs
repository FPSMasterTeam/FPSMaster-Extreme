...
/// device-code enabled, no secret) and set `FPSMASTER_MS_CLIENT_ID` to its GUID,
...
/// The client id to use, overridable via `FPSMASTER_MS_CLIENT_ID` so the user can
/// match the app that issued their refresh token (a refresh token only redeems
/// against its own client id).
pub fn client_id() -> String {
    std::env::var("FPSMASTER_MS_CLIENT_ID").unwrap_or_else(|_| DEFAULT_MS_CLIENT_ID.to_owned())
}

/// The Azure tenant segment of the endpoint URLs (`consumers` for personal
/// accounts, `common` for both), overridable via `FPSMASTER_MS_TENANT`.
fn tenant() -> String {
    std::env::var("FPSMASTER_MS_TENANT").unwrap_or_else(|_| "consumers".to_owned())
}
...
/// The fpsmaster config directory (`<config dir>/fpsmaster/`), shared by the
...
/// Path of the accounts file: `<config dir>/fpsmaster/accounts.json`.
