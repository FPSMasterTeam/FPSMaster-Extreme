...
struct App {
    ...
    entity_model: fpsmaster_render::ModelMesh,
    ...
    entity_glint: fpsmaster_render::ModelMesh,
...
struct WinitApp {
    ...
    app: Option<App>,
    atlas_uv: fpsmaster_render::AtlasUv,
    overlay_textures: fpsmaster_render::OverlayTextures,
    // Input state:
...
impl WinitApp {
    fn new(config: LaunchConfig) -> Self {
        ...
        Self {
            ...
            overlay_textures: fpsmaster_render::OverlayTextures::load(),
...
impl ApplicationHandler for WinitApp {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        let mut app = App {
            ...
            ext: fpsmaster_ext::ExtManager::new(),
            block_tints: fpsmaster_render::TintTable::new(),
            tints_dirty: false,
        ...
        if std::env::var("FPSMASTER_EXT_DEMO").is_ok() {
                ...
                if app.screen.is_some() {
                    ...
                    if let Some(name) = key_name(&event.physical_key) {
                        if app.ext.dispatch_input(
                            &fpsmaster_ext::InputEvent::new(name, pressed),
                            &GameViews(&app.game),
                        ) {
    ...
    fn about_to_wait(&mut self, event_loop: &ActiveEventLoop) {
        ...
        // tick). KeepAlive / ConfirmTransaction pongs + teleport-acks therefore go
        // out spread across real time at the frame rate, like vanilla. Draining
        // them per tick (20 Hz) instead batched several real-time-spread
...
fn main() -> anyhow::Result<()> {
    ...
    if let Some(path) = &config.assets {
        env::set_var("FPSMASTER_ASSET_PATH", path);
    }
...
fn handle_actions(
    ...
    atlas_uv: &mut fpsmaster_render::AtlasUv,
) {
...
fn apply_ext_commands(app: &mut App) {
    for cmd in app.ext.take_commands() {
        match cmd {
            fpsmaster_ext::ExtCommand::Chat(s) => {
                app.pending_ext_packets
                    .push(ServerboundPacket::ChatMessage { message: s });
            }
            fpsmaster_ext::ExtCommand::SendServerbound(b) => {
                app.pending_ext_packets
                    .push(ext_bridge::build_to_serverbound(b));
            }
            fpsmaster_ext::ExtCommand::Log(level, msg) => {
                let level = match level {
                    fpsmaster_ext::LogLevel::Error => log::Level::Error,
                    fpsmaster_ext::LogLevel::Warn => log::Level::Warn,
                    fpsmaster_ext::LogLevel::Info => log::Level::Info,
                    fpsmaster_ext::LogLevel::Debug => log::Level::Debug,
                };
                log::log!(level, "[ext] {msg}");
            }
            fpsmaster_ext::ExtCommand::SpawnParticle {
                kind,
                x,
                y,
                z,
            ...
            fpsmaster_ext::ExtCommand::PlaySound {
                event,
                x,
                y,
                z,
            ...
            fpsmaster_ext::ExtCommand::Render(preset) => apply_render_preset(app, preset),
            fpsmaster_ext::ExtCommand::SubmitGeometry {
                vertices,
                indices,
                texture,
            } => {
                app.ext_geometry = vertices
                    .iter()
                    .map(|v| fpsmaster_render::ModelVertex {
            ...
            fpsmaster_ext::ExtCommand::RegisterTexture {
                handle,
                width,
                height,
                rgba,
            ...
            fpsmaster_ext::ExtCommand::LoadTexture { handle, path } => {
                match std::fs::read(&path)
                    .ok()
                    .and_then(|b| fpsmaster_render::texture::decode_png(&b))
                {
                    Some(img) => {
                        app.mod_textures.insert(handle, std::sync::Arc::new(img));
                    }
            ...
            fpsmaster_ext::ExtCommand::UpdateTexture { handle, rgba } => {
            ...
            fpsmaster_ext::ExtCommand::FreeTexture { handle } => {
                app.mod_textures.remove(&handle);
            }
            fpsmaster_ext::ExtCommand::SetPostEffect { wgsl } => {
                app.pending_post_effect = Some(Some(wgsl));
            }
            fpsmaster_ext::ExtCommand::ClearPostEffect => {
                app.pending_post_effect = Some(None);
            }
            fpsmaster_ext::ExtCommand::RegisterBlock {
                id,
                texture,
                opaque,
                alpha,
                ...
                let tint = match tint {
                    Some(rgb) => fpsmaster_core::Tint::Rgb(rgb),
                    None => fpsmaster_core::Tint::None,
                };
                fpsmaster_core::blocks::register_block(
                    id,
                    fpsmaster_core::blocks::BlockDef::mod_cube(texture, opaque, alpha, tint),
                    luminance,
                );
            }

            // ---- world / player interactions ----
            fpsmaster_ext::ExtCommand::PlaceBlock {
                x,
                y,
                z,
                face,
            ...
            fpsmaster_ext::ExtCommand::Digging {
                status,
                x,
                y,
                z,
                ...
                app.pending_ext_packets.push(ext_bridge::build_to_serverbound(
                    fpsmaster_ext::PacketBuild::PlayerDigging {
                        status,
                        x,
                        y,
                        z,
            ...
            fpsmaster_ext::ExtCommand::AttackEntity { id } => {
                let packets = app.game.ext_attack_entity(id);
                app.pending_ext_packets.extend(packets);
            }
            fpsmaster_ext::ExtCommand::InteractEntity { id, at } => {
                let packets = app.game.ext_interact_entity(id, at);
                app.pending_ext_packets.extend(packets);
            }
            fpsmaster_ext::ExtCommand::ContainerClick { slot, button, mode } => {
                let packets = app.game.container_click(slot, button, mode);
                app.pending_ext_packets.extend(packets);
            }
            fpsmaster_ext::ExtCommand::ContainerClose => {
                if let Some(packet) = app.game.container_close() {
                    app.pending_ext_packets.push(packet);
                }
            }
            fpsmaster_ext::ExtCommand::OpenInventory => app.game.open_player_inventory(),
            fpsmaster_ext::ExtCommand::SelectSlot { slot } => {
                let packets = app.game.ext_select_slot(slot);
                app.pending_ext_packets.extend(packets);
            }
            fpsmaster_ext::ExtCommand::SwingArm => {
                let packets = app.game.ext_swing();
                app.pending_ext_packets.extend(packets);
            }
            fpsmaster_ext::ExtCommand::UseItem => {
                let packets = app.game.ext_use_item();
                app.pending_ext_packets.extend(packets);
            }
            fpsmaster_ext::ExtCommand::SetRotation { yaw, pitch, silent } => {
            ...
            fpsmaster_ext::ExtCommand::ClearSilentRotation => {
                app.game.ext_clear_silent_rotation();
            }
            fpsmaster_ext::ExtCommand::SaveConfig { dir, json } => {
...
fn filter_serverbound(app: &mut App, packets: Vec<ServerboundPacket>) -> Vec<ServerboundPacket> {
    ...
    for packet in packets {
        ...
        if !matches!(verdict, fpsmaster_ext::Verdict::Drop) {
            kept.push(packet);
        }
    }
    kept
...
fn apply_render_preset(app: &mut App, preset: fpsmaster_ext::RenderPreset) {
    use fpsmaster_ext::RenderPreset as P;
    match preset {
        P::BlockTint {
            block_id,
            meta,
...
fn build_debug_lines(app: &App) -> Vec<fpsmaster_render::LineVertex> {
    let mut out = Vec::new();
    let game = &app.game;
    ...
    if app.ext_chunk_borders {
        ...
        let v = |x: f32, y: f32, z: f32| fpsmaster_render::LineVertex {
            position: [x, y, z],
            color,
        };
...
fn build_debug_tris(app: &App) -> Vec<fpsmaster_render::LineVertex> {
    let mut out = Vec::new();
    if let Some(rgb) = app.ext_entity_box {
        ...
        for e in GameViews(&app.game).entities() {
            let (hw, h) = match e.kind {
                fpsmaster_ext::EntityKindView::Player | fpsmaster_ext::EntityKindView::Mob(_) => {
                    (0.3, 1.8)
                }
                _ => (0.15, 0.3),
            };
...
fn push_box(out: &mut Vec<fpsmaster_render::LineVertex>, min: [f32; 3], max: [f32; 3], color: [f32; 4]) {
    let v = |x: f32, y: f32, z: f32| fpsmaster_render::LineVertex {
        position: [x, y, z],
        color,
    };
...
fn push_solid_box(out: &mut Vec<fpsmaster_render::LineVertex>, min: [f32; 3], max: [f32; 3], color: [f32; 4]) {
    let p = [
    ...
    for [a, b, c, d] in FACES {
        for i in [a, b, c, a, c, d] {
            out.push(fpsmaster_render::LineVertex {
                position: p[i],
                color,
            });
        }
...
fn push_tube_box(out: &mut Vec<fpsmaster_render::LineVertex>, min: [f32; 3], max: [f32; 3], t: f32, color: [f32; 4]) {
    let h = t * 0.5;
    let [x0, y0, z0] = min;
    let [x1, y1, z1] = max;
    for y in [y0, y1] {
...
/// over the vanilla HUD. `fpsmaster_ext` authors coordinates in GUI pixels; the
...
fn replay_hud(
    ...
    draw: &fpsmaster_ext::HudDraw,
    scale: i32,
    textures: &std::collections::HashMap<u32, std::sync::Arc<image::RgbaImage>>,
) {
    use fpsmaster_ext::HudCmd;
    use fpsmaster_render::{UiColor, UiRect};
        ...
        match network.events.try_recv() {
            ...
            Ok(NetworkEvent::ChunkColumn { x, z, column }) => {
                ...
                app.ext.dispatch_event(
                    &fpsmaster_ext::ExtEvent::ChunkLoad { x, z },
                    &GameViews(&app.game),
                );
            }
            Ok(NetworkEvent::ChunkUnload { x, z }) => {
                ...
                app.ext.dispatch_event(
                    &fpsmaster_ext::ExtEvent::ChunkUnload { x, z },
                    &GameViews(&app.game),
                );
            }
...
fn append_mesh(
    vertices: &mut Vec<fpsmaster_render::Vertex>,
    indices: &mut Vec<u32>,
    mesh: (Vec<fpsmaster_render::Vertex>, Vec<u32>),
) {
...
fn build_inventory_preview(
    ...
    use fpsmaster_core::EntityKind;
    use fpsmaster_render::EntityAnim;
    ...
    *last_frame = now;
    ...
    if app.in_world {
        ...
        let (particle_v, particle_i) =
            fpsmaster_render::build_particle_mesh(&app.game.camera, &particles);
        ...
        let (orb_v, orb_i) = fpsmaster_render::build_particle_mesh(&app.game.camera, &orbs);
        renderer.set_xp_orbs(&orb_v, &orb_i);
    } else {
        app.last_entity_key = None;
        renderer.upload_model(&fpsmaster_render::ModelMesh::new());
        renderer.set_arm_index_start(None);
        renderer.set_entity_glint(&fpsmaster_render::ModelMesh::new());
        renderer.set_first_person_item(&[], &[]);
        renderer.set_first_person_item_glint(&[], &[]);
    ...
    if hud_visible {
        let scale = gui::gui_scale(width, height);
        let hud_ctx = fpsmaster_ext::HudCtx {
            width: width / scale,
            height: height / scale,
            scale,
            screen_open,
        };
        let mut hud_draw = fpsmaster_ext::HudDraw::new();
    ...
    if !has_panorama {
        ...
        let mut player_body = fpsmaster_render::ModelMesh::new();
...
impl LaunchConfig {
    fn from_args() -> Self {
        let mut server = None;
        let mut username = "FPSMaster".to_owned();
        let mut assets = None;
...
fn run_headless_interact(config: &LaunchConfig, seconds: f32) -> anyhow::Result<()> {
    ...
    loop {
        ...
        // finds a ground block, sweeping pitch a little if needed. FPSMASTER_DIG_DOWN
        ...
        let dig_down = std::env::var("FPSMASTER_DIG_DOWN").is_ok();
        if in_game {
