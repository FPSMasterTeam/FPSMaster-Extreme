...
use fpsmaster_render::{text_width, UiFrame, UiRect};
...
impl GuiAccounts {
    ...
    fn layout(&mut self, ctx: &DrawCtx) {
        ...
        self.buttons = vec![
            GuiButton::at_px(x, row, half, s, tr("fpsmaster.accounts.use")).disabled(!has_selection),
            GuiButton::at_px(x + step, row, half, s, tr("fpsmaster.accounts.addMicrosoft")),
            GuiButton::at_px(x + 2 * step, row, half, s, tr("fpsmaster.accounts.addToken")),
            // Second row: delete / copy token / back.
            GuiButton::at_px(x, row - 24 * s, half, s, tr("selectServer.delete")).disabled(!has_selection),
            GuiButton::at_px(x + step, row - 24 * s, half, s, tr("fpsmaster.accounts.copyToken")),
...
impl GuiScreen for GuiAccounts {
    ...
    fn draw(&mut self, ui: &mut UiFrame, ctx: &DrawCtx) {
        ...
        let s = ctx.scale;
        draw_centered_text(ui, ctx.width, 16 * s, s, TEXT_WHITE, &tr("fpsmaster.accounts.title"));
        ...
        for (visible_index, index) in (self.scroll..total).take(visible as usize).enumerate() {
            ...
            if account.active {
                let tag = format!("§a{}", tr("fpsmaster.accounts.inUse"));
...
impl GuiAddToken {
    ...
    fn layout(&mut self, ctx: &DrawCtx) {
        ...
        self.buttons = vec![
            GuiButton::at_px(bx, btn_y, 98 * s, s, tr("fpsmaster.accounts.add")),
...
impl GuiScreen for GuiAddToken {
    ...
    fn draw(&mut self, ui: &mut UiFrame, ctx: &DrawCtx) {
        ...
        let s = ctx.scale;
        let top = ctx.height / 4;
        draw_centered_text(ui, ctx.width, top - 24 * s, s, TEXT_WHITE, &tr("fpsmaster.accounts.addTitle"));
        draw_centered_text(
            ui,
            ctx.width,
            top + 4 * s,
            s,
            TEXT_GRAY,
            &tr("fpsmaster.accounts.addInstr"),
        );
