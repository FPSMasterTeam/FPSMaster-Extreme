...
pub struct DrawCtx<'a> {
    ...
    pub mods: &'a [fpsmaster_ext::ModInfo],
}
...
pub(crate) fn draw_tooltip(
    ...
    let text_w = lines
        .iter()
        .map(|l| fpsmaster_render::text_width(l, s))
        .max()
        .unwrap_or(0);
...
pub(crate) fn draw_centered_text(
    ...
    let w = fpsmaster_render::text_width(text, scale);
...
pub(crate) fn fit_text(text: &str, max_width: i32, scale: i32) -> String {
    if fpsmaster_render::text_width(text, scale) <= max_width {
    ...
    for ch in text.chars() {
        ...
        if fpsmaster_render::text_width(&candidate, scale) > max_width {
            break;
        }
        out.push(ch);
    }
