...
use fpsmaster_render::{UiColor, UiFrame, UiRect};
...
impl GuiScreen for GuiModList {
    ...
    fn draw(&mut self, ui: &mut UiFrame, ctx: &DrawCtx) {
        ...
        let s = ctx.scale;
        draw_centered_text(ui, ctx.width, 8 * s, s, TEXT_WHITE, &tr("fpsmaster.mods.title"));
        let count = ctx.mods.len();
        let subtitle = if count == 0 {
            tr("fpsmaster.mods.empty")
        } else {
            let enabled = ctx.mods.iter().filter(|m| m.enabled).count();
            tr_args("fpsmaster.mods.enabledCount", &[&enabled.to_string(), &count.to_string()])
        };
        ...
        for vi in 0..visible {
            ...
            let state = if m.enabled { format!("§a{}", tr("fpsmaster.mods.on")) } else { format!("§7{}", tr("fpsmaster.mods.off")) };
            ...
            let detail = match &m.description {
                ...
                _ => m
                    .tier
                    .map(|t| format!("{t:?} mod"))
                    .unwrap_or_else(|| tr("fpsmaster.mods.builtin")),
            };
        ...
        let reload = GuiButton::at_px(cx - 154 * s, by, 150 * s, s, tr("fpsmaster.mods.reload"));
        let open = GuiButton::at_px(cx + 4 * s, by, 150 * s, s, tr("fpsmaster.mods.openFolder"));
