...
use fpsmaster_render::{discover_resource_packs, PackMeta, UiFrame, UiRect};
...
impl GuiScreen for GuiResourcePacks {
    ...
    fn draw(&mut self, ui: &mut UiFrame, ctx: &DrawCtx) {
        ...
        for vi in 0..visible {
            ...
            if idx == 0 {
                let label = fit_text(&tr("fpsmaster.resourcepack.default"), max_w, s);
                ui.text_shadowed(text_x, text_y, s, TEXT_WHITE, &label);
                ui.text_shadowed(text_x, text_y + 12 * s, s, TEXT_GRAY, &tr("fpsmaster.resourcepack.none"));
            } else {
                let pack = &self.packs[idx as usize - 1];
                let label = fit_text(&pack.name, max_w, s);
                ui.text_shadowed(text_x, text_y, s, TEXT_WHITE, &label);
                let info = tr_args("fpsmaster.resourcepack.format", &[&pack.meta.pack_format.to_string(), &pack.meta.description]);
