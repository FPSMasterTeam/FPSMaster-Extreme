...
impl ItemRenderer {
        ...
        if (0..256).contains(&item.id) {
            ...
            match block.render_shape() {
                ...
                RenderShape::Cross | RenderShape::Rail | RenderShape::Ladder => {
                    ...
                    push_sprite(&mut vertices, &mut indices, &to_world, name.as_deref(), atlas, tint, fpsmaster_render::FULLBRIGHT);
                }
                _ => {
                    ...
                    finish_item_model(&mut m);
                    let to_world = |c: Vec3| view_to_world(camera, m.transform_point3(c));
                    push_block_cube(&mut vertices, &mut indices, &to_world, block, atlas, fpsmaster_render::FULLBRIGHT, 0.0);
                }
            }
    ...
    /// fpsmaster's feet-up, front +z arm frame by a 180-degree rotation about X
...
fn held_item_to_world(h: &PlayerHeldItem, kind: HeldItemKind) -> impl Fn(Vec3) -> Vec3 + '_ {
    ...
    move |c: Vec3| {
        // Vanilla authored item space (blocks, y-down, front −z) → fpsmaster
...
mod tests {
    ...
    use fpsmaster_protocol::v1_8_9::packets::SlotItem;
    use fpsmaster_render::TextureAtlasImage;
