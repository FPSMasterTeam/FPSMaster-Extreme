...
const SETTINGS_FILE: &str = "fpsmaster_options.txt";
...
impl Settings {
    ...
    pub fn resolution_label(self) -> String {
        match self.resolution {
            None => crate::i18n::tr("fpsmaster.options.native"),
...
/// inventory/chat/hotbar set fpsmaster actually uses, plus the vanilla controls
/// fpsmaster does not yet act on (drop, swap-hands, screenshot, perspective) so
...
impl GameAction {
    ...
    fn default_key(self) -> KeyCode {
        match self {
            ...
            // Attack/Use are mouse buttons in vanilla; fpsmaster drives them from
...
/// The keys fpsmaster can bind, paired with their `Debug`/serialization name. This
...
mod tests {
    ...
    fn settings_round_trip_through_disk() {
        let path = std::env::temp_dir().join("fpsmaster_settings_round_trip.txt");
    ...
    fn missing_file_yields_defaults() {
        let path = std::env::temp_dir().join("fpsmaster_settings_does_not_exist.txt");
        let _ = std::fs::remove_file(&path);
        let loaded = Settings::load_from(&path);
    ...
    fn out_of_range_values_are_clamped() {
        let path = std::env::temp_dir().join("fpsmaster_settings_clamp.txt");
    ...
    fn keybinds_persist_through_disk() {
        let path = std::env::temp_dir().join("fpsmaster_settings_keybinds.txt");
        let _ = std::fs::remove_file(&path);
