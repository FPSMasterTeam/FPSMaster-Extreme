...
pub const ENTITY_SLOT_COUNT: u32 = 51;
...
pub enum EntitySlot {
    ...
    ChestTrapped = 42,
    ChestEnder = 43,
    /// Double-chest textures (entity/chest/{normal,trapped}_double.png, 128x64):
    /// fill the full 128px slot width, sampled by the large-chest model.
    ChestNormalDouble = 44,
    ChestTrappedDouble = 45,
    ...
    Arrow = 48,
    /// Wither skeleton (skeleton/wither_skeleton.png, 64x32) — only its head
    /// region is sampled, by the wither-skeleton skull block-entity.
    SkeletonWither = 49,
...
const MOB_SLOT_ASSETS: [(EntitySlot, &str, [u8; 3]); 39] = [
    ...
    (EntitySlot::ChestNormalDouble, "chest/normal_double", [162, 114, 51]),
    (EntitySlot::ChestTrappedDouble, "chest/trapped_double", [162, 80, 51]),
    ...
    (EntitySlot::SkeletonWither, "skeleton/wither_skeleton", [50, 50, 50]),
...
fn candidate_asset_paths() -> Vec<PathBuf> {
    ...
    if let Some(path) = env::var_os("FPSMASTER_ASSET_PATH") {
        candidates.push(PathBuf::from(path));
    }
    if let Some(path) = env::var_os("FPSMASTER_ASSET_ZIP") {
        candidates.push(PathBuf::from(path));
    }
    if let Some(path) = env::var_os("FPSMASTER_MINECRAFT_JAR") {
        candidates.push(PathBuf::from(path));
    }
...
mod tests {
    ...
    fn slot_origins_are_distinct_and_inside_the_atlas() {
        let slots = [
            ...
            EntitySlot::ChestNormalDouble,
            EntitySlot::ChestTrappedDouble,
            EntitySlot::Sign,
            EntitySlot::EnchantBook,
            EntitySlot::Arrow,
            EntitySlot::SkeletonWither,
            EntitySlot::White,
        ];
        let mut seen = std::collections::HashSet::new();
        for slot in slots {
