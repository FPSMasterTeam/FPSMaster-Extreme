...
use glam::{Mat4, Vec3};
use fpsmaster_core::{ChunkPos, SectionPos, World};
use thiserror::Error;
...
// the dynamic-mesh uploader, the post-effect type, small per-frame helpers),
// split into a child module to keep this file focused on the Renderer itself.
mod gpu;
use gpu::*;
...
pub struct Renderer {
    ...
    last_break_overlay: Option<(i32, i32, i32, u8, fpsmaster_core::BlockState)>,
    ...
    /// Camera-relative render origin (whole blocks) the cached shadow matrix was
    /// built against. The matrix is camera-relative, so when the player crosses a
    /// block boundary `origin_f` jumps and a stale cached matrix would misalign the
    /// shadows by a block — visible as flicker while running/jumping. Forcing a
    /// re-render whenever this changes keeps the sampled origin consistent.
    cached_shadow_origin: glam::IVec3,
    ...
    /// Debug blit of the motion-vector buffer over the swapchain (env FPSMASTER_MV).
    mv_debug: bool,
    ...
    /// resolved frame (copied from `taa_tex` each frame). Env FPSMASTER_TAA.
    taa_enabled: bool,
    ...
    /// Temporary per-pass skip toggles for profiling (FPSMASTER_SKIP env var).
...
impl Renderer {
    pub fn new(window: Arc<Window>) -> Result<Self, RendererError> {
        ...
        let device_descriptor = wgpu::DeviceDescriptor {
            ...
            // create a device. fpsmaster uses no compute, so the adapter's own limits
        ...
        // Water shader: screen-space reflection (`reflect_ray`) + refraction
        // (`refract_water`) + their group(3) bindings come from the prepended
        // water_ssr.wgsl. Both sample the copied scene, so reflected/refracted blocks
        ...
        let mut renderer = Self {
            ...
            cached_shadow_origin: glam::IVec3::ZERO,
            render_scale: 1.0,
            render_distance: 12,
            ...
            mv_view: None,
            mv_bind_group: None,
            mv_debug: std::env::var("FPSMASTER_MV").is_ok(),
            mv_debug_pipeline,
            mv_debug_layout,
            mv_debug_bind_group: None,
            jitter_enabled: std::env::var("FPSMASTER_JITTER").is_ok(),
            jitter_index: 0,
            jitter_offset: glam::Vec2::ZERO,
            taa_enabled: std::env::var("FPSMASTER_TAA").is_ok(),
            taa_pipeline,
            taa_layout,
            taa_tex: None,
            taa_view: None,
    ...
    fn refresh_flat_meshing(&mut self) -> bool {
        // Only the full PATH TRACER (rt_quality 3 / FPSMASTER_PT) forces smooth (non-greedy)
        ...
        let pt_env = std::env::var("FPSMASTER_PT").map(|v| v == "1").unwrap_or(false);
    ...
    /// True while the experimental full path tracer is the active mode. `FPSMASTER_PT=1`
    /// forces it on regardless of the quality setting (debug / quick toggle).
    fn path_tracing(&self) -> bool {
        let forced = std::env::var("FPSMASTER_PT").map(|v| v == "1").unwrap_or(false);
        ...
        if on {
            log::warn!(
                "DLSS requested but this build lacks the `dlss` feature; falling back to FSR/TAA. \
                 Rebuild with `--features fpsmaster_render/dlss` on an RTX + Vulkan box (see docs/DLSS.md)."
            );
        }
        ...
        if self.dlss_active() {
            if let Some(dlss) = &self.dlss {
                let scale = match self.dlss_quality_mode {
                    ...
                    // Auto: pick Quality-like for ≤1440p (fpsmaster's typical target).
    ...
    pub fn set_break_overlay(&mut self, overlay: Option<(i32, i32, i32, u8, fpsmaster_core::BlockState)>) {
    ...
    fn render_inner(
        ...
        // Re-render every other frame, BUT always re-render when the camera-
        // relative origin changed (the player crossed a block): the cached matrix
        // is built in that origin's frame, so reusing it after the origin jumps
        // shifts the whole shadow by a block — the flicker seen while running and
        // jumping. Forcing a redraw there keeps the cached matrix's origin equal to
        // the origin the chunk shader samples with.
        let origin_changed = origin_i != self.cached_shadow_origin;
        let redraw_shadow = render_shadow_map && (self.shadow_tick % 2 == 0 || origin_changed);
        ...
        if redraw_shadow {
            self.cached_shadow_origin = origin_i;
        ...
        if let Some(rt) = self.ray_tracer.as_mut() {
            ...
            // Debug knob: FPSMASTER_GI_DEBUG=<n> exaggerates the GI bounce so per-block
            // colour bleeding is unmistakable (verifies the per-triangle colour lookup).
            let gi_debug = std::env::var("FPSMASTER_GI_DEBUG")
                .ok()
        ...
        if ssr_water && !water_draws.is_empty() && !self.debug_skip.water {
            ...
            // Screen-space reflection + refraction (samples the copied, textured scene,
            // so reflected/refracted blocks keep their real texture — RT's flat
            // per-triangle colour was wrong for water and for cutout flowers/leaves).
            ...
            && self.taa_tex.is_some()
        {
            ...
            // DLSS's reprojection off-screen → black (so fpsmaster's `cur-prev` sign is
    ...
    fn prepare_ui(&mut self, ui: &UiFrame) {
        ...
        for item in ui.block_items() {
            crate::gui_item::append_block_icon(
                &mut vertices,
                &mut indices,
                fpsmaster_core::BlockState::new(item.block_id, item.meta),
                item.dst,
                surface,
                &self.atlas_uv,
                &self.biome_colors,
        ...
        for item in ui.glint_items() {
            match item.block {
                Some((block_id, meta)) => crate::gui_item::append_block_icon(
                    &mut glint_v,
                    &mut glint_i,
                    fpsmaster_core::BlockState::new(block_id, meta),
                    item.dst,
                    surface,
                    &self.atlas_uv,
                    &self.biome_colors,
