...
pub fn clickable_spans(segments: &[ChatSegment], max_width: i32, scale: i32) -> Vec<Vec<ClickSpan>> {
    ...
    for glyph in &glyphs {
        ...
        let advance = recraft_render::char_width(glyph.c, scale);
        if width + advance > max_width && width > 0 {
            ...
            rows.push(std::mem::take(&mut row));
            row = carried;
            width = row.iter().map(|g| recraft_render::char_width(g.c, scale)).sum();
            last_space = None;
        }
        if glyph.c == ' ' {
...
mod tests {
    ...
    fn clickable_spans_locate_the_clicked_segment() {
        ...
        assert_eq!(span.x_start, recraft_render::text_width("hi ", 1));
        assert_eq!(span.x_end, recraft_render::text_width("hi [go]", 1));
