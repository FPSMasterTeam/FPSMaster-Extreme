...
use recraft_render::{GuiTexture, UiFrame, UiRect};
...
pub fn draw_offer(ui: &mut UiFrame, container: &Container, px: i32, py: i32, scale: i32, mouse: (f64, f64)) {
    ...
    let icon = 16 * scale;
    let mut preview = |pos: (i32, i32), item: &recraft_protocol::v1_8_9::packets::SlotItem| {
...
pub fn draw_tooltip(ui: &mut UiFrame, container: &Container, ctx: &DrawCtx, px: i32, py: i32, scale: i32) -> bool {
    ...
    if item_hover(BUY1) {
        tip(ui, &recraft_render::build_tooltip(&recipe.buy));
    } else if recipe.buy_secondary.as_ref().is_some_and(|_| item_hover(BUY2)) {
        tip(ui, &recraft_render::build_tooltip(recipe.buy_secondary.as_ref().unwrap()));
    } else if item_hover(SELL) {
        tip(ui, &recraft_render::build_tooltip(&recipe.sell));
    ...
    {
        tip(ui, &["Can't trade with villager at the moment".to_string()]);
    } else {
        return false;
    }
    true
