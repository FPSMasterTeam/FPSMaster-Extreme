...
//!
//! It is intentionally decoupled from the rest of fpsmaster (no core/protocol/
//! render/app dependency). The host (`fpsmaster_app`) performs every projection
//! into and out of these plain types. That decoupling is what makes the same
//! vocabulary usable by the JS bridge and — once mirrored in `fpsmaster_ext_api` —
...
mod tests {
    ...
    fn native_mod_loads_and_runs() {
        ...
        // bare `cargo test -p fpsmaster_ext` doesn't fail; run after
        // `cargo build -p fpsmaster_native_example`.
        ...
        let name = format!("{prefix}fpsmaster_native_example.{ext}");
        ...
        // an older fpsmaster_ext_api layout would be rejected by abi_stable — that
        ...
        let Some(lib) = lib else {
            eprintln!("skipping native_mod_loads_and_runs: build fpsmaster_native_example first");
            return;
        };
        let adapter = native::NativeAdapter::load(&lib).expect("load native mod");
        assert_eq!(adapter.id(), "fpsmaster.native.example");
        let mut mgr = ExtManager::new();
    ...
    fn load_mods_from_dir_in_dependency_order() {
        ...
        let dir = std::env::temp_dir().join(format!("fpsmaster_ext_modtest_{}", std::process::id()));
