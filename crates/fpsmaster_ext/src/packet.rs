...
//! are deliberately *projections* of the internal `fpsmaster_protocol` enums — the
//! host (`fpsmaster_app`) converts in both directions — so the public surface
...
pub enum PacketType {
    ...
    TimeUpdate,
    Disconnect,
    /// Any clientbound packet fpsmaster does not model (`raw_id` carries the id).
    ClientboundOther,
