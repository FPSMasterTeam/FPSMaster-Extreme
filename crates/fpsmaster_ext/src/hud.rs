...
//! (`fpsmaster_app`) replays them into its `fpsmaster_render::UiFrame`. Going through
//! a plain command list (rather than handing out `&mut UiFrame`) is what lets the
//! exact same HUD API cross the native ABI boundary later, and keeps `fpsmaster_ext`
