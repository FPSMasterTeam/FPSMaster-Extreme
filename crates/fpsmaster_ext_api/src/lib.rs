//! Stable plugin ABI for fpsmaster native (`cdylib`) extensions.
//!
//! This is the obfuscation-exempt contract a native mod compiles against. It
//! deliberately depends only on `abi_stable` — never on fpsmaster internals — so
...
/// `use fpsmaster_ext_api::prelude::*;` and implement [`NativePlugin`].
pub mod prelude {
    pub use crate::{
...
impl HostApi {
    ...
    /// Register a content-mod full-cube block at `id` (fpsmaster-authoritative
...
impl RootModule for ExtApiRef {
    ...
    const BASE_NAME: &'static str = "fpsmaster_ext_mod";
    const NAME: &'static str = "fpsmaster_ext_mod";
